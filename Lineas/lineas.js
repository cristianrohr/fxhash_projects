let positions = [];
let cs = 1000;
let pt, lamount, lamountlabel, pv, ext, pvl, size, borderw, lt, ltl, bg, scap, scapl;
...
function setup() {
    ...
    rn = fxrand();
    size = rn < 0.25 ? 1 : rn < 0.6 ? round(fxr(2,4)) : round(fxr(5,8));
    ...
    rn = fxrand();
    lamount = rn < 0.05 ? round(fxr(2000, 3000)) : rn < 0.2 ? round(fxr(1200, 1500)) : rn < 0.4 ? round(fxr(600, 800)) : round(fxr(350, 400));
...
function fxr(_e, _t) {
    return Array.isArray(_e) ? _e[round(fxrand() * _e.length)] : (void 0 === _t && (_t = _e, _e = 0), fxrand() * (_t - _e) + _e)
}
